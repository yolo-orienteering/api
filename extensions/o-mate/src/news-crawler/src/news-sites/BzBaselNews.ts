...
// bz Basel (bzbasel.ch) runs on the same CH Media publishing platform as
// Aargauer Zeitung, so the article-link and detail-page structure is identical.
// The configured path is a general sports section rather than an OL-filtered
// search, so the keyword filter below does the heavy lifting of discarding
// unrelated articles.
export class BzBaselNews extends NewsSiteAdapter {
  BASE_URL: string = 'https://www.bzbasel.ch'
  ...
  async listNews(): Promise<void> {
    ...
    $('a[href*="ld."]').each((_, el) => {
      ...
      if (href) {
        ...
        // Structure in teasers: <span data-timestamp-span="...">DD.MM.YYYY</span>
        const dateText =
  ...
  async downloadANews(urlWithDate: UrlList): Promise<void> {
    ...
    // Extract Date if not already found from list. bz Basel (CH Media) does NOT
    // expose `article:published_time`; its machine-readable publication date is
    // in `<meta name="date">` and `<time datetime="…">`. Fall back to a
    // dd.MM.yyyy pattern in the article text only as a last resort.
    if (!urlWithDate.date) {
