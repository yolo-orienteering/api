...
import { Schema, RaceInstruction } from '../../types/DirectusTypes'
...
export default class InstructionAI {
  ...
  private async processInstructionsWithAI(): Promise<void> {
    ...
    if (instructions.length !== this.instructionsWithNewFiles.length) {
      throw new Error(
        `Unexpected Error: Loaded instructions do not equal the expected amount.`,
      )
    }
  ...
  private async askAI({
    ...
    if (!fileId) {
      console.warn(
        `No AI file id available for instruction with id ${instruction.id}`,
      )
      return
    }
  ...
  private async uploadFileToAI(instruction: RaceInstruction): Promise<void> {
    try {
      ...
      const fileHash = await this.generateHashAndCompare(
        arrayBuffer,
        instruction,
      )
      ...
      if (!fileId) {
        console.error(
          `File upload failed for instruction with id ${instruction.id}`,
        )
        return
      }

      // store file id and hash
      const updatedInstruction = await this.instructionsService.updateOne(
        instruction.id,
        {
          fileIdAI: fileId,
          fileHash,
  ...
  private async generateHashAndCompare(
    ...
    if (!generatedHash) {
      console.warn(
        `Could not generate hash for instruction id ${instruction.id}`,
      )
      return false
    }
    if (generatedHash === storedHash) {
      console.warn(
        `Instruction file with id ${instruction.id} already uploaded to OpenAI.`,
      )
      return false
    }
