...
export class SolvDepartures extends Crawler implements ICrawler {
  ...
  private async getCategoriesOfRacesAndSave(): Promise<void> {
    ...
    for (const race of this.racesHavingDepartures) {
      ...
      const categoriesWithDepartures = await this.normalizeCategoriesOfRace(
        html,
        race,
      )
  ...
  private async normalizeCategoriesOfRace(
    html: string,
    race: Race,
  ): Promise<RawCategoryWithDepartures[]> {
      ...
      if (nextPreTag.length > 0) {
        ...
        if (match) {
          ...
          const raceCategory: Partial<RaceCategory> = {
            name: categoryName,
            distanceInMeter: distanceKm
              ? parseFloat(distanceKm) * 1000
              : undefined, // Convert km to meters
            equidistanceInMeter: equidistanceM
              ? parseFloat(equidistanceM)
              : undefined, // Already in meters
            amountOfControls: controls ? parseInt(controls, 10) : undefined, // Convert to integer
            race: race.id,
          }

          // get departures
          const rawDepartures: RawDeparture[] = lines
            .map((line) => {
              const match = line.match(
                /^\s*\d+\s+(.+?)\s+(\d{2})\s+(.+?)\s+(.+?)\s+(\d{1,2}:\d{2})$/,
              )
              if (match) {
                const [
                  ,
  ...
  private async saveData(
    ...
    const rawRaceCategories: Partial<RaceCategory>[] =
      categoriesWithDepartures.map((categoryWith) => categoryWith.raceCategory)
    const raceCategories = await this.saveRaceCategories(
      race,
      rawRaceCategories,
    )
    ...
    await this.saveDepartures({
      categoriesWithDepartures,
      raceCategories,
      users: departureUsers,
      race,
    })
  }
  ...
  private async saveDepartures({
    ...
    for (const raceCategory of categoriesWithDepartures) {
      ...
      for (const departure of raceCategory.departures) {
        const relatedRaceCategory = raceCategories.find(
          (tmpRaceCategory) =>
            tmpRaceCategory.name === raceCategory.raceCategory.name,
        )

        if (!relatedRaceCategory) {
          console.warn(
            'Could not find related race category, but should exist.',
          )
          continue
        }
        ...
        const relatedUser = users.find(
          (tmpUser) => tmpUser.composedIdentifierSolv === userIdentifier,
        )
        if (!relatedUser) {
    ...
    if (existingDepartures.length) {
      newUserDepartures = newUserDepartures.map((newUserDeparture) => {
        const existingUserDeparture = existingDepartures.find(
          (existingDeparture) => {
            return (
              existingDeparture.user === newUserDeparture.user &&
  ...
  private async saveUsers(
    categoriesWithDepartures: RawCategoryWithDepartures[],
  ): Promise<CustomDirectusUser[]> {
    console.log('save users...')
    let users: Partial<CustomDirectusUser>[] = []
    ...
    for (const categoryWithDepartures of categoriesWithDepartures) {
      for (const departure of categoryWithDepartures.departures) {
        const [firstName, ...lastNameParts] =
          departure.firstAndLastName?.split(' ') || []
        users.push({
          composedIdentifierSolv:
            this.getUserIdentifierFromRawDeparture(departure),
    ...
    const existingUsers = (await this.usersService.readByQuery({
      filter: {
        composedIdentifierSolv: {
          _in: users.map(
            (userToSave) => userToSave.composedIdentifierSolv,
          ) as string[],
        },
      },
    ...
    if (existingUsers.length) {
      users = users.map((user) => {
        const existingUser = existingUsers.find(
          (existingUser) =>
            existingUser.composedIdentifierSolv === user.composedIdentifierSolv,
        )
        if (!existingUser) {
          return user
        }
  ...
  private async saveRaceCategories(
    race: Race,
    raceCategories: Partial<RaceCategory>[],
  ): Promise<RaceCategory[]> {
    ...
    const existingRaceCategories =
      (await this.raceCategoriesService.readByQuery({
        filter: {
          _and: [
            {
              race: {
    ...
    if (existingRaceCategories.length) {
      raceCategories = raceCategories.map((raceCategory) => {
        const existingRaceCategory = existingRaceCategories.find(
          (existing) => existing.name == raceCategory.name,
        )
