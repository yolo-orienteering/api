...
export default class SolvDecorator {
  ...
  private async loadSite(): Promise<string | false> {
    ...
    const response: AxiosResponse = await axios.get(
      `${this.BASE_URL}${this.FIXTURES_PATH}${originalDataId}`,
      {
  ...
  private decorateWithLinks($: cheerio.CheerioAPI): void {
    $('a').each((_index, value) => {
      ...
      // inscriptionLink
      const hasInscriptionLink = this.LINK_IDENTIFIER.inscriptionLink.find(
        (inscriptionLink: string) => text.includes(inscriptionLink),
      )
